...
<body>
	...
	<div class="container">
		<br> <br>
		<h2 style="text-align: center; color: red">Danh sách đơn đặt hàng
			của bạn</h2>
		<br>
		<p style="text-align: center; color: green"><u>${msgSuccessOrderr}</u></p>
		
		<p style="text-align: center; color: blue"><u>${msgFail}</u></p>
		
		<p style="text-align: center; color: blue"><u>${msgDeleOrderSuccess}</u></p>
		...
		-->
			<c:forEach var="o" items="${listOrOf}" varStatus="status">
				<h3>Đơn đặt hàng thứ ${status.index+1}:</h3>
				<br>
				<br>
				<c:forEach var="d" items="${listOrDetail}">
					<c:if test="${d[0] ==o.id}">
						<ul>
							<li>Mã đơn hàng: ${o.id}</li>
							<li>Ngày đặt hàng: ${o.created}</li>
							<li>Tổng số lượng các món hàng: ${d[2]}</li>
							<li>Tổng tiền phải trả: $${d[1]}</li>
							<li>Trạng thái đơn hàng: <font color="blue"><u>${o.status.name}</u></font></li>
							<li><a href="client/order/remove/${o.id}.htm"><font
									color="red">Hủy đơn hàng</font></a></li>
							<li><a href="client/orderDetail/${o.id}.htm"><font
									color="red">Chi tiết đơn hàng</font></a></li>
