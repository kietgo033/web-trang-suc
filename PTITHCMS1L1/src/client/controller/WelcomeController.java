		...
		for (User ktra : listAcc) {
			if (u.getUsername().equals(ktra.getUsername()) && u.getEmail().equals(ktra.getEmail())) {
				...
				try {
					...
					t.commit();
					// mailer.send("tckietct@gmail.com", userP.getEmail(), "Đổi
					// mật khẩu thành công!",
					// "Mật khẩu mới là: "+kq);
					...
					t.rollback();
					//model.addAttribute("failForgetPass", "Đổi mật khẩu thất bại!");
					//System.out.print("edit password thất bại!");
					//return "admin/forget";
				...
				// gửi email
				try {
					mailer.send("tckietct@gmail.com", userP.getEmail(), "Đổi mật khẩu thành công",
							"Cảm ơn bạn đã sử dụng dịch vụ tại Website Kietgo <br> Mật khẩu mới là: "  + kq
									+ " .<br>Chân thành cảm ơn quý khách.");
					// model.addAttribute("mgsMail", "Gửi mail thành công!");
					System.out.print("Gửi email thanh cong!");
				} catch (Exception e) {
					model.addAttribute("msgMail", "Lỗi gửi email thất bại!!");
					System.out.print("gửi email khong thanh cong");
					//return "admin/forget";
					// TODO: handle exception
				}
