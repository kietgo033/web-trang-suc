...
public class checkoutController {
	...
	public String Checkout(ModelMap model, HttpSession session) {
		...
		try {
			mailer.send("tckietct@gmail.com", user.getEmail(), "Đặt hàng thành công",		
					"Cảm ơn bạn đã đặt hàng tại Website Kietgo.<br>Tổng số tiền bạn phải trả là: "+"$"+price+" .<br>Món hàng của bạn đang được chuẩn bị. Chúng tôi sẽ liên hệ với bạn sớm nhất!<br><br> Chân thành cảm ơn quý khách.");
...
//		try 
//		{
//			sessionn.update(order);
//			t.commit();
//		} catch (Exception e) {
//			// TODO: handle exception
//			t.rollback();
//			//model.addAttribute("msgDeleOrderFail", "Xóa thất bại");
//			return "redirect:/client/orderList.htm";
//		}
//		finally {
//			sessionn.close();
//		}
	...
	public String getProductList(ModelMap model, @PathVariable("orderId") int orderId) {
		//List<OrderDetail> listOrderDetail = listOrder(orderId);
		//List<Product> listProduct = new ArrayList<>();
//		for(OrderDetail i : listOrderDetail) {
//			listProduct.add(getProduct(i.getProduct().getId()));
//		}
...
//	//lấy sản phẩm theo productId của OrderDetail
//	public Product getProduct(int ProductId) {
//		Session session = factory.getCurrentSession();
//		String hql = "SELECT * FROM Product WHERE id = :id";
//		Query query = session.createQuery(hql);
